...
| 📸 스크린샷 이중화 | 스크린샷을 파일로 저장하면서 동시에 클립보드에도 자동 복사 — 저장과 붙여넣기 사이에 설정을 오갈 필요가 없음 |
...
| 📸 Screenshot doubling | Screenshots are saved to a file and copied to the clipboard at the same time — no more switching the save target back and forth |
...
- 메뉴 → "스크린샷을 다운로드 폴더에 저장하기" — `com.apple.screencapture` 저장 위치를 다운로드로 변경 (최초 1회)
